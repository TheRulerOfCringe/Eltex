...
1. epoll
    ...
    1.4 Добавляем событие
        ...
        1.4.1 Событие "ввод с квалиатуры" у родительского процесса
        ...
        1.4.2 Собатие "собщение от дочернего процесса" у родительского процесса
        ...
        1.4.3 Событие "сообщение от родительского процесса" у дочернего процесса
        ...
        1.4.4 Событие "прозвенел таймер" у дочернего процесса
...
1. token сразу в int без проверок на корректность (ENTER приводит к "Segmentation fault (core dumped)")
...
2. Не передавайтте в таймер отрицательные числа, пж пж пж пж пж

ИСПРАВЛЕНО:

Так и отрицательные числа в timer не пускаем, и при введении символов ругаемся.
...
3. Как будто нужен отдельный BUFFER_SIZE для сообщений-команд, а то 1024 жирно.
...
Но вообще-то там обычно не больше 4 байт занимает

4. Я забыл, что просто дочерний процесс может писать свой статус по запросу от родителя

А НАДО ЛИ ИСПРАВЛЯТЬ:

Поэтому родитель шлёт "s", дочерний процесс шлёт "s 0" или "s 1", родитель это парсит и печатает в консоль.
Хотя просто дочерний процесс мог бы писать в консоль...
Но так должно быть дале лучше, монополия на взаимодействие с пользователем у родительского процесса.

5. ЧТО ПО FREE_DRIVER?
