...
int (*select_operation(int choice)) (int, int)
{
    switch (choice)
    {
...
int int_calc()
{
    ...
    while (flag)
    {
        ...
        if (fgets(buffer, sizeof(buffer), stdin) != NULL)
        {
            ...
            {
                int (*operation)(int, int) = select_operation(choice);
...
float (*fselect_operation(int choice)) (float, float)
{
    switch (choice)
    {
        // case 1 нет потому что в дефолт ушла :)
...
float float_calc()
{
    ...
    while (flag)
    {
        ...
        if (fgets(buffer, sizeof(buffer), stdin) != NULL)
        {
            ...
            if (sscanf_s(buffer, "%d %c", &choice, &extra, 1) == 1)
            {
                float (*operation)(float, float) = fselect_operation(choice);
                return operation(a, b);
            }
            else
...
int main()
{
    // я это навайбкодил и это не работает
    
    WIN32_FIND_DATA findData;
    HANDLE hFind = FindFirstFile("C:\\Users\\ilyat\\source\\repos\\Eltex_6.3\\Eltex_6.3\\libs\\*.dll", &findData);

    if (hFind != INVALID_HANDLE_VALUE)
    {
        do {
            char path[100];
            sprintf_s(path, sizeof(path), "./libs/%s", findData.cFileName);

            HMODULE handle = LoadLibrary(path);
            if (handle) {
                char func_name[50];
                strcpy_s(func_name, sizeof(func_name), findData.cFileName);
                func_name[strlen(func_name) - 4] = '\0'; // remove .dll
